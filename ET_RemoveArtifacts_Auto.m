function S = ET_RemoveArtifacts_Auto(S, varargin)
%Algorithm to automatically remove most artifacts in a pupillary signal.
%
...
%presses the pushbutton "Filter Blinks". 
%
%This function can also be called as a standalone function (to be
...
for k=1:config.iterations
    ...
    for j=1:length(blink_index.onset)
        if timestamp(blink_index.offset(j))-timestamp(blink_index.onset(j)) > 10
            %don't do anything if interpolation region is greater than 10
            %seconds
        else
